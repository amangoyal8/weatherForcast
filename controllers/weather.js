...
exports.getWeather = async (req, res) => {
  ...
  try {
    ...
    const mongoData = await db.findOne({ date: endDate.toISOString().split('T')[0], city: req.city }).lean();
    if (mongoData) {
      ...
      const forecastUrl = `http://api.weatherapi.com/v1/forecast.json?key=${apiKey}&q=${Data.city}&days=4`;
      ...
      for (const date of previousDates) {
        ...
        const historyUrl = `http://api.weatherapi.com/v1/history.json?key=${apiKey}&q=${Data.city}&dt=${date}`;
